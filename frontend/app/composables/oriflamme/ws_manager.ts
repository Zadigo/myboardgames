import type { CardActions, GameRegistry, Nullable } from '~/types'
...
export type ActionOptions = {
  create_game: [ { playerUuid: Nullable<string> } ]
  start_game: [ { playerUuid: Nullable<string>, tableUuid: Nullable<string> } ]
...
export type ResponseOptions = {
  must_identify: { playerUuid: Nullable<string> }
  create_game: { gameRegistry: GameRegistry }
  place_card: { gameRegistry: GameRegistry }
  start_game: { gameRegistry: GameRegistry }
}
...
export const useOriflammeComposable = createGlobalState(() => {
  ...
  const playerCards = computed(() => {
    const cards = cardsInPlay.value.filter(card => card.owner.uuid === playerUuid.value)
    return cards.filter(card => !card.inQueue)
  })
