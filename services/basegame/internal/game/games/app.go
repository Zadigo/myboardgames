...
// GameApp is the main game server application that manages the lifecycle of a game instance.
// It is responsible for handling player interactions, managing game state, and enforcing game rules.
// The GameApp can be started and stopped independently of the main server application.
type GameApp struct {
...
// Deprecated: Start initializes and starts the game server application.
...
func (app *GameApp) GetCurrentPlayer() (client *WebsocketClient, err error) {
...
func (app *GameApp) AddPlayer(client *WebsocketClient) error {
	if client == nil {
...
func NewGame(ctx context.Context, options models.GameAppOptions) *GameApp {
	return &GameApp{
		ctx:                    ctx,
		Uuid:                   uuid.NewString(),
		CurrentRound:           0,
