...
func TestCardConstructors(t *testing.T) {
	...
	t.Run("Has 45 cards", func(t *testing.T) {
...
func TestCard(t *testing.T) {
	...
	t.Run("Card cannot be revealed with no queue and no owner", func(t *testing.T) {
		...
		state, err := card.Reveal(&backend.InfluenceQueue{}, backend.PlayerChoices{})
...
// func TestCardInvalidActions(t *testing.T) {
...
func TestArcherCard(t *testing.T) {
	...
	for i, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Equal(t, "Archer", currentCard.GetBaseCard().Name)
			result, err := currentCard.Reveal(simpleQueue, backend.PlayerChoices{FirstCard: tc.firstCard})
